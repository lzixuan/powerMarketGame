...
locIdx_name = {0: 'City South', 1: 'City North'}
loadProfile = {
    0: [300, 500, 650, 450],
    1: [50, 60, 70, 40]
}
...
def showMarketInfo(roles):
    with use_scope('market', clear=True):
        ...
        loadDf['City South'] = loadProfile[0]
        loadDf['City North'] = loadProfile[1]
        fig2 = px.bar(loadDf, x='Period', y=['City South', 'City North'])
...
def showBids(period):
    ...
    if len(bids_period[period]) > 0:
        ...
        fig.add_vline(x=lmp_loc0, line_dash='dash', line_color='firebrick', annotation_text='LMP_CitySouth', annotation_position='top left')
        fig.add_vline(x=lmp_loc1, line_dash='dash', line_color='firebrick', annotation_text='LMP_CityNorth', annotation_position='top right')
        html = fig.to_html(include_plotlyjs="require", full_html=False)
        put_html(html)

def showDispatch(period, roles):
    put_text(f'Locational Marginal Price (Market Clearing Price, $/MWh):\nCity South: {clearingPrice[period - 1][0]}, City North: {clearingPrice[period - 1][1]}')
    tableHeader = ['Role', 'Fuel', 'Location', 'Player', 'Bid Capacity (MW)', 'Dispatch Result (MW)', 'Accum. Revenue ($)', 'Accum. Profit ($)']
    tableContent = []
    for i in range(1, 7):
        if i in roleID_gameID.keys():
            gameID = str(roleID_gameID[i])
        ...
        roleID = i
        role = roles[str(roleID)]
        row = [str(roleID), role['Fuel'], locIdx_name[role['Location']], gameID, str(sum([bid.amount for bid in bids_period[period] if bid.id == roleID])), str(dispatchRes[i][period - 1]), str(revenue[i]), str(profit[i])]
        tableContent.append(row)
    put_table(tableContent, header=tableHeader)
        ...
        if period <= 4:
            ...
            if period <= 4:
                for i in range(1, 7):
                    role = roles[str(i)]
                    if role['Fuel'] == 'wind':
                        renewBidLimit[str(i)] = role['Nameplate Capacity (Maximum possible generation MW)'] * windProfile[period - 1]
                    elif role['Fuel'] == 'solar':
                        renewBidLimit[str(i)] = role['Nameplate Capacity (Maximum possible generation MW)'] * solarProfile[period - 1]
